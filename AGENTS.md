...
│   │   └── widgets/      # Widgets d'ère : passifs + 18 interactifs (BohrAtom, StarNursery, PeriodicTable, AccretionDisk, MoleculeBuilder, PetriDish, SingularityWidget de fin...) routés par interactive.ts ; clic d'ère via useEraMechanic (hook propre aux widgets, vit ici et non dans src/hooks/) ; descriptif partagé via WidgetHint (texte centré + WidgetGalet) ; helpers svgCoords.ts, StarField.tsx
