...
│   │   └── widgets/      # Widgets d'ère : passifs + 10+ interactifs (BohrAtom, StarNursery, PeriodicTable, AccretionDisk, MoleculeBuilder, PetriDish, SingularityWidget de fin...) routés par interactive.ts ; clic d'ère via useEraMechanic ; descriptif partagé via WidgetHint (texte centré + WidgetGalet) ; helpers svgCoords.ts, StarField.tsx
