...
export function UnificationWheel({ era }: { era: EraDef }): ReactElement {
  ...
  return (
    <div className="mt-2 flex w-full flex-col items-center gap-2">
      ...
      <div
        className="relative aspect-9/4 w-full max-w-2xl"
        role="group"
        aria-label={verb}
      >
        <svg
        ...
        >
          ...
          {pulses.map((p) => (
            <circle key={p.key} r="2.2" fill="var(--color-accent)">
              <animate attributeName="cx" from={p.x} to={CX} dur={`${PULSE_MS}ms`} fill="freeze" />
              <animate attributeName="cy" from={p.y} to={CY} dur={`${PULSE_MS}ms`} fill="freeze" />
              <animate attributeName="opacity" from="0.9" to="0" dur={`${PULSE_MS}ms`} fill="freeze" />
            </circle>
          ))}
        </svg>
